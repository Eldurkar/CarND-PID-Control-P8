...
int main() {
  ...
  //pid_steer.Init(0.135, 0.0002, 3);
  pid_steer.Init(0.15, 0.00001, 2.5);
  //pid_throttle.Init(0.316731, 0.00001, 0.0226185);
  pid_throttle.Init(0.316, 0.0, 0.0225);

  
  h.onMessage([&pid_steer, &pid_throttle](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, 
                     uWS::OpCode opCode) {

  // Original starteer code
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          throttle_value = 0.3
                           - pid_throttle.Kp * pid_throttle.p_error
                           - pid_throttle.Kd * pid_throttle.d_error
                           - pid_throttle.Ki * pid_throttle.i_error;
          // Avoid stopping and reverse
          if (speed < 18.0) //throttle_value < 0.1 && 
          {
            throttle_value = 1.0;            
          }
